...
const SYSTEM_DEFS = [
    ...
    { name:"7G-QIG",  region:"wd" }, { name:"8RQJ-2",  region:"wd" },
...
export default function RaidableSkyhooks() {
  ...
  const [globalNameCache, setGlobalNameCache] = useState({});
  ...
  // Keep ref for cache tracking inside effects
  ...
  // Global ESI Name Resolvers
  useEffect(() => {
    if (currentFilter !== 'global' || !rawSkyhooks.length) return;

    async function resolveGlobalNames() {
      const unknownIds = [...new Set(rawSkyhooks
        .map(h => h.solar_system_id)
      ...
      try {
        const res = await fetch('https://esi.evetech.net/latest/universe/names/?datasource=tranquility', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify(unknownIds.map(id => parseInt(id)))
        });
        if (res.ok) {
          const names = await res.json();
          const newCache = { ...nameCacheRef.current };
          names.forEach(n => { newCache[parseInt(n.id)] = n.name; });
          setGlobalNameCache(newCache);
        }
      } catch (e) {
        console.error("Global name cache build failure", e);
      }
    }
    resolveGlobalNames();
  }, [currentFilter, rawSkyhooks]);
  ...
  const hookMap = {};
  rawSkyhooks.forEach(h => { hookMap[h.solar_system_id] = h; });
  ...
  return (
    <div className="skyhook-dashboard-wrapper" style={{
      ...
      {/* Scope style sheets inject safe wrapper variables scoping CSS pollution inside documentation */}
      <style>{`
...
<div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={i} style={{ background: '#111318', border: '1px solid #1e2330', borderRadius: '4px', padding: '10px 16px', minWidth: '120px', flex: '1' }}>
            {/* Note the camelCase change to letterSpacing below */}
            <label style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#5a6278', display: 'block', marginBottom: '4px' }}>
              {st.label}
            </label>
            <span className="mono-text" style={{ fontSize: '20px', fontWeight: '600', color: st.color }}>
              {st.val}
            </span>
          </div>
        ))}
      ...
      {isStale && (
        <div style={{ display: 'flex', gap: '12px', background: 'rgba(255,148,0,0.08)', border: '1px solid rgba(255,148,0,0.35)', borderRadius: '5px', padding: '12px 16px', marginBottom: '16px', fontSize: '13px', color: '#ff9900' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: '#8a94a8', margin: 0, lineHeight: 1.5 }}>None of our tracked alliance systems appear in the current ESI feed ({rawSkyhooks.length} returned globally). Windows only register on ESI when active or highly imminent.</p>
          </div>
        </div>
      )}

      {missingIds.length > 0 && (
        <div className="mono-text" style={{ fontSize: '11px', color: '#ff9900', marginBottom: '10px', padding: '8px 12px', background: 'rgba(255,148,0,0.06)', border: '1px solid rgba(255,148,0,0.2)', borderRadius: '4px' }}>
          ⚠ Excluded unresolvable systems: {missingIds.join(', ')}
        </div>
      )}
